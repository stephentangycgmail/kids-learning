(function () {
  ...
  function mode() { return document.querySelector('input[name="choiceMode"]:checked').value; }
  ...
  function renderSetup() { el.choiceTopic.textContent = ""; topics.forEach((topic) => { const option = document.createElement("option"); option.value = topic.id; option.textContent = `${topic.label} · ${topic.labelZh}`; el.choiceTopic.appendChild(option); }); updateSetup(); }
  function updateSetup() { const topic = currentTopic(); const quiz = mode() === "choice_quiz"; el.choiceStart.textContent = quiz ? "Start Quiz (10)" : "Start Practice (12)"; el.learningLink.href = `grammar.html?lesson=${encodeURIComponent(topic.id)}`; }
  function queueSave() { session.lastSavedAt = new Date().toISOString(); el.choiceSave.textContent = "Saved"; return Promise.resolve(); }
  function begin() { const topic = currentTopic(); const selectedMode = mode(); const count = selectedMode === "choice_quiz" ? 10 : 12; try { const questions = core.selectChoiceQuestions(questionsForTopic(topic), count); session = core.createChoiceSession({ mode: selectedMode, topic, questions }); showSession(); } catch (cause) { error("This topic does not have enough questions.", cause); } }
